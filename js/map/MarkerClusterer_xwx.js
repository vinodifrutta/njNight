...
/**
 * update by xwx ,基于原版进行性能优化及适应性改造
 */
...
function removeOverlay(map,marker){
	if(marker.getLabel){
...
(function(){
    ...
    MarkerClusterer.prototype._removeMarkersFromMap = function(){
        for(var i = 0, marker; marker = this._markers[i]; i++){
            marker.isInCluster = false;
            removeOverlay(this._map,marker);
        }
    };
     ...
     * @return 无返回值
     */
    MarkerClusterer.prototype.redraw = function (markers) {
    	if(markers){
    		this._markers=[];
    		if(isArray(markers)){
    			for(var i = 0, len = markers.length; i <len ; i++){
    ...
    Cluster.prototype.updateClusterMarker = function () {
        if (this._map.getZoom() > this._markerClusterer.getMaxZoom()) {
            this._clusterMarker && removeOverlay(this._map,this._clusterMarker);
            for (var i = 0, marker; marker = this._markers[i]; i++) {
        ...
        this._clusterMarker._markers=this._markers;
    ...
    Cluster.prototype.remove = function(){
        for (var i = 0, m; m = this._markers[i]; i++) {
                this._markers[i].getMap() && removeOverlay(this._map,this._markers[i]);
        }//清除散的标记点
        removeOverlay(this._map,this._clusterMarker);
